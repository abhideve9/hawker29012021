...
//print_r ($_REQUEST);
  if(isset($_REQUEST['id']))
  {
    ...
    //$message=$_REQUEST['message'];
      ...
      $UpdateQuery="UPDATE tbl_request_for_hawker_advertisement SET  aproval_status='1', modified_user_name='$adminName' WHERE id='".$_REQUEST['id']."'";
      $UpdateResult=mysqli_query($db,$UpdateQuery);
      if($UpdateResult)
      {

        $ssql=mysqli_query($db,"SELECT hawker_code FROM tbl_request_for_hawker_advertisement WHERE id='$id'");
        $fetchsql=mysqli_fetch_assoc($ssql);
        $hawker_code=$fetchsql['hawker_code'];
        //echo "SELECT notification_id FROM login_manage_seller WHERE user_id='$hawker_code'AND active_status=1";exit;
        ...
        $firebase = new Firebase();
         ...
         ?>
        <script>
          alert("free ads approved successfully");
