   ...
   public function login_post()
    {
	  ...
	  /* if($rowdata1['active_status']=='1')
       ...
       $response->data = $result;
		 ...
		 $data['message'] = 'Coming soon in your area. Currently this service is active in Gurgaon only.';
	 ...
	 public function seller_profile_post()
	  {
		...
		if(!empty($categoryResult))
		   ...
		   //print_r($catStringString);exit;
            ...
            {
             $data1->total_call ='0';
             $data1->totalclick=MIN($bannerClickResult,$bannerClickResultFree);
              $data1->totalCategoryclick=$catReesult;
			...
			$data1->id=$id;
		  	...
		  	if($active_status1=='0')
		  	{
		  		...
		  		$data1->city_message='Coming soon in your area. Currently this service is active in Gurgaon only.';
		  	}
...
public function getPlanType_get()
  ...
  public function getHawkersVisitedInformation_GET()
  {
		require APPPATH . 'firebase.php';
		require APPPATH . 'push.php';
		require APPPATH . 'config.php';

  	$result=$this->Seller->getAllHawker();

  	foreach($result as $value)
  	{
  		$mobile_no=$value['mobile_no_contact'];
  		$hawker_code=$value['hawker_code'];
  		$resdata = 					$this->Seller->check_total_count_call($hawker_code);
  		$bannerClickResult = 		$this->Seller->check_total_count_bannerClick($hawker_code);
  		$bannerClickResultFree = 	$this->Seller->check_total_count_bannerClickFree($hawker_code);
        ...
        if(!empty($categoryResult))
        {
				...
				for($i=0;$i<count($categoryResult);$i++)
				{
					...
					$catString2 .=$categoryResult[$i]['cat_id']==0? :$categoryResult[$i]['cat_id'].',';

				}
				
				$catReesult=$this->Seller->getFixerCategoryCountRecord2(rtrim($catString2,','));

				//echo $catReesult.'<br>';

				$notification_id=$this->Seller->getNotificationData($hawker_code);

				if(!empty($notification_id))
				{
				pushNotification($notification_id,MAX($bannerClickResult,$bannerClickResultFree),$catReesult,$resdata);
				}
		}
  ...
  public function locationOffDutyOff_post()
  {
  try
   	 {
  		 ...
  		 if(!empty($device_id))
  		 {
	  		 $hawker_code=$this->Seller->getHawkerCodeUsingDeviceId($device_id);
			 //print_r($hawker_code);exit;
			 if(!empty($hawker_code))
	  		 {
	  		      $return=$this->Seller->dutyONOFF($hawker_code);
			      require APPPATH . 'firebase.php';
			      require APPPATH . 'push.php';
			      require APPPATH . 'config.php';
			      $firebase = new Firebase();
			      ...
			      $regId =$this->Seller->getNotificationData($hawker_code);
			      //sleep(10);
 ...
 public function getThirtyMinutsIntervalHawkers_get()
 {
 	try
 	{
 		...
 		$date=date('Y-m-d');
		$date='2020-12-29';
 		$result=$this->Seller->getThirtyMinutsIntervalHawkers($date);
 		// print_r($result);exit;
 		if(!empty($result))
 		{
 				foreach($result AS $value)
 				{
 					$hawker_code=$value['user_id'];
 					$response=$this->Seller->checkDuty($hawker_code);
 					if(!empty($response))
 					{
					      require APPPATH . 'firebase.php';
					      require APPPATH . 'push.php';
...
public function fakeEntry_get()
{
	$date=date('Y-m-d');
    	$daysName=date('l', strtotime($date));

	for($i=0;$i<=($daysName=='Sunday'||$daysName=='Saturday'?mt_rand(3,15):mt_rand(3,13));$i++)
	{
		$this->Seller->insertPaidFakeEntry($date);
		
	}
	for($i=0;$i<=($daysName=='Sunday'||$daysName=='Saturday'?mt_rand(1,11):mt_rand(1,9));$i++)
	{
		
		$this->Seller->insertFreeFakeEntry($date);
		
	}
	for($i=0;$i<=($daysName=='Sunday'||$daysName=='Saturday'?mt_rand(2,12):mt_rand(4,11));$i++)
	{
		
		$this->Seller->insertCategoryFakeEntry($date);
	}
