 ...
 public function verification_otp_data_post()
     {
      ...
      else
      {
        ...
        $referral_point_update=$this->Customer->update_status_referral_point($referal_code,$mobile_no,$referral_point);
...
*/
    ...
    /* print_r("select * from tbl_genrate_referral_code_for_customer where device_id='".$device_id."' and mobile_no='".$mobile_no."'");
     die();*/
     /*$row = $que->num_rows();*/
  ...
  public function banner_image_list_data_post()
     ...
     public function count_customer_referral_data_post()
    {
     ...
     date_default_timezone_set('Asia/kolkata'); 
     $now = date('Y-m-d');
     $query = $this->db->query("SELECT SUM(referral_point) from tbl_genrate_referral_code_for_customer where mobile_no='$mobile_no' and referral_flag_status='1' and limit_status!='1'");
     $current_data=$query->result_array();

      $query1 = $this->db->query("SELECT SUM(customer_point) from tbl_genrate_referral_code_for_customer where mobile_no='$mobile_no' and referral_flag_status='1' and limit_status!='1'");
   ...
   public function request_customer_referral_data_post()
   {
     ...
     $resdata = $this->Customer->get_referral_voucher_data($data1);
    
     $type=$resdata->type;
     $amount=$resdata->Amount;
     $coupon_code=$resdata->coupon_code;
     $expire_date=$resdata->expire_date;
     $data->coupon_code=$coupon_code;
     if(!empty($resdata))
     {
       $res = $this->Customer->request_customer_referral_money_data($data);
       $res2 = $this->Customer->update_referral_data($mobile_no);
     }
