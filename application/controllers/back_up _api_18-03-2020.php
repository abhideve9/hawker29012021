 ...
 class Api extends CI_Controller {
  ...
  /////////////////////////  Cron job ///////////////////////////////
   public function notification_by_customer()
   {
    ...
    $cat_data=$this->User->customer_cat_data($cat_id);
    $cat_data=$cat_data->cat_name;
    }
    else
    {
     $getdata = $this->User->check_data_by_registerseller1($sub_cat_id,$city);
      $sub_cat_data=$this->User->customer_sub_cat_data($sub_cat_id);
   ...
   if(!empty($getdata))
   {
       ...
       foreach ($getdata_temp as $row_temp)
    {
     ...
     if($distance<=$radius)
     {
      ...
      $title ='Hawkers';
      // notification message
      $message ='We have found '. $cat_data2 .' hawker near by you';
      // push type - single user / topic
      $push_type = 'individual';
      $push->setTitle($title);
      $push->setMessage($message);
       ...
       foreach ($getdata_seasonal as $row_seasonal)
    {
     ...
     if($distance<=$radius)
     {
      ...
      $title ='Hawkers';
      // notification message
      $message ='We have found '. $cat_data3 .' hawker near by you';
      // push type - single user / topic
      $push_type = 'individual';
      $push->setTitle($title);
      $push->setMessage($message);
      ...
      if($userType=='Moving')
    {
     ...
     if($distance<$radius)
     {
      ...
      $title ='Hawkers';
      // notification message
      $message ='We have found '. $cat_data1 .' hawker near by you';
      // push type - single user / topic
      $push_type = 'individual';
      $push->setTitle($title);
      $push->setMessage($message);
      ...
      if($distance<$radius)
    {
      ...
      $title ='Hawkers';
      // notification message
      $message ='We have found '. $cat_data4 .' hawker near by you';
      // push type - single user / topic
      $push_type = 'individual';
      $push->setTitle($title);
      $push->setMessage($message);
    ...
    foreach ($getdata2 as $row2)
    {
      ...
      if($distance<$radius)
     {
      ...
      $title ='Hawkers';
      // notification message
      $message ='We have found '. $cat_data5 .' hawker near by you';
      // push type - single user / topic
      $push_type = 'individual';
      $push->setTitle($title);
      $push->setMessage($message);
  ...
  public function notification_by_hawker()
   {
    $notifiedhawker = $this->User->hawker_get_data();

   //print_r($notifiedhawker);
    foreach($notifiedhawker as $rowdata)
    {
    $end_date=$rowdata['end_date'];
    $result = DateTime::createFromFormat("d/M/Y", $end_date)->format("d/m/Y");
    $result1 = strtotime($result);
   
    $hawker_code=$rowdata['hawker_code'];
    //print_r($hawker_code);
    $get_hawker_notification_id = $this->User->get_hawker_notification_id($hawker_code);
    ...
    if($now1>$result1)
    {
    foreach ($get_hawker_notification_id as $rownotified_data)
     {
  ...
  //  print_r($business_start_time);
   ...
   // print_r($get_hawker_notification_id);
    ...
    if($now == $business_start_time)
    {
      ...
      $title ='Hawkers';
      // notification message
    $message ='Turned on Duty? /क्या आप ड्यूटी स्टार्ट करना भूल गए हो? ';
      // push type - single user / topic
      $push_type = 'individual';
      $push->setTitle($title);
      $push->setMessage($message);
    ...
    /*.....  Cron job for notification by after end time Seasonal and Temporary Hawker  ---- */
     ...
     /*......Cron job for notification for  duty on by  Hawker for fix user ---- */
   public function notification_by__duty_on_for_fix_hawker()
   {
   $notifiedhawker_by_duty_on_data_fix = $this->User->notifiedhawker_by_duty_on_fix();
   foreach($notifiedhawker_by_duty_on_data_fix as $rowdata)
    {
    $business_start_time=$rowdata['business_start_time'];
    $duty_status=$rowdata['duty_status'];
  //  print_r($business_start_time);
   ...
   // print_r($get_hawker_notification_id);
    ...
    /*......Cron job for notification for  duty Off by  Hawker ---- */
  public function notification_by_hawker_for_duty_off()
   {
   $notifiedhawker_by_duty_off_data = $this->User->notifiedhawker_by_duty_off();
   foreach($notifiedhawker_by_duty_off_data as $rowdata)
    {
    $business_close_time=$rowdata['business_close_time'];

    $duty_status=$rowdata['duty_status'];
    //print_r($business_close_time);
    $hawker_code=$rowdata['hawker_code'];
    // print_r($hawker_code);
    $get_hawker_notification_id_data = $this->User->get_hawker_notification_id($hawker_code);
    date_default_timezone_set('Asia/Kolkata');
    ...
    if($now == $business_close_time)
    {
      ...
      $title ='Hawkers';
      // notification message
     $message ="Turned Off Duty? /क्या आप ड्यूटी बंद करना भूल गए हो?";
      // push type - single user / topic
      $push_type = 'individual';
      $push->setTitle($title);
      $push->setMessage($message);
    ...
    /*.....  Cron job for notification by after end time Seasonal and Temporary Hawker  ---- */
     ...
     /*......Cron job for notification for  duty Off by  Hawker ---- */
  public function notification_for_duty_off_by_fix_hawker()
   {
   $notifiedhawker_by_duty_off_data_fix = $this->User->notifiedhawker_by_duty_off_fix();
   foreach($notifiedhawker_by_duty_off_data_fix as $rowdata)
    {
    $business_close_time=$rowdata['business_close_time'];
  ...
  else
  {
    ...
    /*.....  Cron job for notification by after end time Seasonal and Temporary Hawker  ---- */
  public function notification_by__duty_on_for_all_hawker()
   {
