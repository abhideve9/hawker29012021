...
{    
  ...
  function check_data_by_registerseller1_temp_moving($sub_cat_id,$city)
    ...
    {  
      $q=$this->db->query("SELECT business_start_time,duty_status,hawker_code,shop_latitude,shop_longitude  from registration_sellers where (verification_status =1 or verification_by_call = 1) and user_type='Fix'");
      return($q->result_array());
    }

    function notifiedhawker_by_duty_on_Seasonals()
    {  
      $q=$this->db->query("SELECT user_type, seasonal_temp_hawker_type,duty_status,hawker_code,shop_latitude,shop_longitude,start_date  from registration_sellers where (verification_status =1 or verification_by_call = 1) and user_type='Seasonal' and seasonal_temp_hawker_type='Fix'");
      return($q->result_array());
    }
