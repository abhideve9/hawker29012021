...
{    
   ...
   /* function history_profile($data23)
    ...
    $query="insert into login_manage_seller(user_id,name,mobile_no,device_id,notification_id,login_time,status,active_status) values('$userId','$name1','$mobile_no','$device_id','$notification_id','$login_time','1','1')";
             $this->db->query($query);
        }
     }
   ...
   /* function check_data_by_registerseller($cat_id)
      ...
      function insert_duty_status__for_seller($data3)
    {
     ...
     public function check_total_count_call($hawker_code)
     {
        date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
        $q=$this->db->query("SELECT hawker_id from tbl_customer_call_by_hawker where hawker_id='".$hawker_code."' and date_time LIKE '%$now%' AND banner_flag=0");
        return($q->num_rows());
     }
...
public function check_total_count_bannerClick($hawker_code)
     {
        date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
        $q=$this->db->query("SELECT * FROM `tbl_customer_call_by_hawker` AS `tcch`INNER JOIN `paid_advertisement_by_merchant`AS `pabm`ON `pabm`.`hawker_code`=`tcch`.`hawker_id`
            WHERE `tcch`.`banner_flag`='1'AND `pabm`.`ads_status`='Ongoing'AND `pabm`.`payment_status`='Paid'AND `pabm`.`aproval_status`='1' AND `tcch`.`hawker_id`=$hawker_code AND DATE_FORMAT(`tcch`.`date_time`,'%Y-%m-%d')=CURDATE() GROUP BY `tcch`.`id`");
//print_r( $this->db->last_query());exit;

      
return($q->num_rows());
     }
public function check_total_count_bannerClickFree($hawker_code)
     {

        $q=$this->db->query("SELECT `tcch`.`hawker_id` FROM `tbl_customer_call_by_hawker` AS `tcch`INNER JOIN `tbl_request_for_hawker_advertisement`AS `tfha`ON `tfha`.`hawker_code`=`tcch`.`hawker_id`
            WHERE `tcch`.`hawker_id`=$hawker_code AND `tcch`.`banner_flag`='1'AND DATE_FORMAT(`tcch`.`date_time`,'%Y-%m-%d')=CURDATE() GROUP BY `tcch`.`id`");
        return($q->num_rows());
     }
function getFixerCategoryCountRecord($hawker_code)
     {
          $this->db->select('mfc.cat_id'); 
          $this->db->FROM('misc_fixer_category AS mfc'); 
          $this->db->WHERE('`mfc`.`hawker_code`',$hawker_code);
          $query=$this->db->get();
          // print_r( $this->db->last_query());exit;
          $result=$query->result_array();
          return $result;
     }
      function getFixerCategoryCountRecord2($catStringString)
     {
	  
         $q=$this->db->query("SELECT * FROM `fixer_count_category`WHERE `cat_id` IN($catStringString) AND creation_date=CURDATE()");
        // print_r( $this->db->last_query());exit;
          return($q->num_rows());
     }
        public function getAllHawker()
        {

                  $this->db->select('rs.hawker_code,rs.mobile_no_contact'); 
                  $this->db->FROM('registration_sellers AS rs'); 
                  $this->db->JOIN('login_manage_seller AS lms','lms.user_id=rs.hawker_code','INNER'); 
                  $this->db->WHERE('lms.active_status', 1);
                  $query=$this->db->get();
                  // print_r( $this->db->last_query());exit;
                  $result=$query->result_array();
                  return $result;
...
public function getNotificationData($hawker_code)
    {
              $this->db->select('notification_id');
              $this->db->from('login_manage_seller');
              $this->db->where('user_id',$hawker_code);
 ...
 public function updateHawkers($hawker_code)
    {
        date_default_timezone_set('Asia/kolkata');
        $this->db->where('hawker_code',$hawker_code);
        $this->db->where('rejection_status',0);
        $this->db->update('tbl_request_for_hawker_advertisement',array('status'=>'0','modified_date'=>date('Y-m-d H:i:s')));
        // print_r($this->db->last_query());exit;
        return $this->db->affected_rows();
    }
 public function getHawkerCodeUsingDeviceId($device_id)
    {

        $this->db->select('seller_id');
        $this->db->from('tbl_otp_seller');
    ...
    public function dutyONOFF($hawker_code)
    {
        $this->db->where('hawker_code',$hawker_code);
        $this->db->update('registration_sellers',array('duty_status'=>0));
	//print_r($this->db->last_query());exit;
         return $this->db->affected_rows();
    }
  public function getThirtyMinutsIntervalHawkers($date)
    {
      
    $query=$this->db->query('SELECT DATE_FORMAT(`gsl`.`registered_time`,"%Y-%m-%d") AS `registered_date`,`lms`.`user_id` FROM     `gps_seller_location` AS `gsl` 
            INNER JOIN `login_manage_seller` AS `lms` ON `lms`.`device_id`=`gsl`.`shop_gps_id` 
            WHERE TIMESTAMPDIFF(MINUTE,`gsl`.`registered_time`, NOW()) > 30 AND `gsl`.`registered_time` LIKE "%'.$date.'%" AND `lms`.`active_status`=1 GROUP BY `gsl`.`shop_gps_id` ORDER BY `gsl`.`registered_time` DESC');
        //print_r($this->db->last_query());exit;
         $result=$query->result_array();
         return $result;
    }
    public function checkDuty($hawker_code)
    {

      $this->db->select('*');
      $this->db->from('registration_sellers');
      $this->db->where('hawker_code',$hawker_code);
      $this->db->where('duty_status',1);
      $query=$this->db->get();
      //print_r( $this->db->last_query());
      $result=$query->result_array();
      return $result;   
...
public function insertPaidFakeEntry($date)
{

    $this->db->query('INSERT INTO `tbl_customer_call_by_hawker` (`hawker_id`,`date_time`,`STATUS`,`banner_flag`) SELECT `hawker_code`,"'.$date.'","1","1" FROM `registration_sellers` WHERE `hawker_code` IN(SELECT `rs`.`hawker_code` FROM `registration_sellers` AS `rs` 
INNER JOIN `login_manage_seller` AS `lms` ON `lms`.`user_id`=`rs`.`hawker_code` 
INNER JOIN `paid_advertisement_by_merchant` AS `pabm` ON `pabm`.`hawker_code`=`rs`.`hawker_code` GROUP BY `rs`.`hawker_code`)');
    

}

public function insertFreeFakeEntry($date)
{
    $this->db->query('INSERT INTO `tbl_customer_call_by_hawker` (`hawker_id`,`date_time`,`STATUS`,`banner_flag`) SELECT `hawker_code`,"'.$date.'","1","1" FROM `registration_sellers` WHERE `hawker_code` IN(SELECT `rs`.`hawker_code` FROM `registration_sellers` AS `rs` 
INNER JOIN `login_manage_seller` AS `lms` ON `lms`.`user_id`=`rs`.`hawker_code`
INNER JOIN `tbl_request_for_hawker_advertisement` AS `rfha` ON `rfha`.`hawker_code`=`rs`.`hawker_code` GROUP BY `rs`.`hawker_code`)');
   
}
public function insertCategoryFakeEntry($date)
{
    $this->db->query('INSERT INTO `fixer_count_category` (`cat_id`,`creation_date`) SELECT `cat_id`,"'.$date.'" FROM `misc_fixer_category` WHERE `hawker_code` IN (SELECT `mfc`.`hawker_code` FROM `misc_fixer_category`AS `mfc` 
INNER JOIN `login_manage_seller` AS `lms` ON `lms`.`user_id`=`mfc`.`hawker_code`)');
