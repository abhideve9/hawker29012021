...
public function check_total_count_bannerClick($hawker_code)
     {
        date_default_timezone_set('Asia/kolkata'); 
        $now = date('Y-m-d');
        $q=$this->db->query("SELECT * FROM `tbl_customer_call_by_hawker` AS `tcch`LEFT JOIN `paid_advertisement_by_merchant`AS `pabm`ON `pabm`.`hawker_code`=`tcch`.`hawker_id`
            WHERE `tcch`.`banner_flag`='1'AND `pabm`.`ads_status`='Ongoing'AND `pabm`.`payment_status`='Paid'AND `pabm`.`aproval_status`='1' AND `tcch`.`hawker_id`=$hawker_code AND DATE_FORMAT(`tcch`.`date_time`,'%Y-%m-%d')=CURDATE()");
//print_r( $this->db->last_query());exit;
...
public function check_total_count_bannerClickFree($hawker_code)
     {
        ...
        $q=$this->db->query("SELECT `tcch`.`hawker_id` FROM `tbl_customer_call_by_hawker` AS `tcch`LEFT JOIN `tbl_request_for_hawker_advertisement`AS `tfha`ON `tfha`.`hawker_code`=`tcch`.`hawker_id`
