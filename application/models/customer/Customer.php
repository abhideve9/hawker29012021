...
{   
    ...
    // echo $mobile_no;exit();
     ...
     {
         //echo "testing";exit();
     $q=$this->db->query("SELECT customer_referral_code, customer_point from tbl_genrate_referral_code_for_customer where mobile_no='$mobile_no' and status='1'");
     // print_r("SELECT customer_referral_code, customer_point from tbl_genrate_referral_code_for_customer where mobile_no='$mobile_no' and customer_referral_code IS NULL and customer_point='0' and status='1'");exit();
       $row=$q->row();
     // print_r($row);exit;
    ...
    //echo "test";exit();
     $up=$this->db->query("UPDATE tbl_genrate_referral_code_for_customer SET referral_point=referral_point+'$coupon_amount', total_point= referral_point+customer_point ,referral_flag_status='1' where genrate_referral_code='$referal_code'");
      ...
      if ($up==true) { 
        ...
        $sql=$this->db->query("SELECT notification_id FROM login_manage_custumer WHERE mobile_no IN ('$mobile_no','$customer_mobile_no') AND active_status=1");

          $fetch=$sql->result_array();
              ...
              // notification title
               $title ='Hawkers';
               $message ='You have earned '.$coupon_amount.' points.';
   ...
   /*   return($q->num_rows());*/
    ...
    {
     //print_r("SELECT mobile_no,referral_point,city,pincode from tbl_genrate_referral_code_for_customer where genrate_referral_code='$referal_code' and customer_point='0'");
      //exit();
        $q=$this->db->query("SELECT mobile_no,referral_point,city,pincode from tbl_genrate_referral_code_for_customer where genrate_referral_code='$referal_code'");
         return($q->row());
        $row = $q->num_rows();
...
public function getRedeemAmount($mobile_no)
 ...
 public function getMerchantForDistance($from,$to,$sales_id)
  ...
  {
   $result=$this->db->insert_batch(TBL_FIXER_COUNT_CATEGORY,$arrayData);
   if($result)
   {
     return $this->db->insert_id();
   }else
 ...
 public function getNotificationData($mobile_no)
    {
 ...
 public function deletePreviousMonthsRecords()
  {

    $this->db->where('creation_date < DATE_SUB(NOW(), INTERVAL 1 MONTH)');
    $this->db->delete('fixer_count_category');
   }
