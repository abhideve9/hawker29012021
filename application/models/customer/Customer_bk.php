...
{    
  ...
  /* function near_by_hawker_data($data)
    ...
    { 
     ...
     function update_status_referral_point($referal_code,$mobile_no,$referral_point,$coupon_amounts)
    {   
      date_default_timezone_set('Asia/kolkata'); 
      $now = date('Y-m-d H:i:s');
       $nows = date('Y-m-d');
      ...
      date_default_timezone_set('Asia/kolkata'); 
      $now = date('Y-m-d H:i:s');
      $this->db->query("UPDATE tbl_genrate_referral_code_for_customer SET referral_point='$referral_point',referral_flag_status='1' where genrate_referral_code='$referal_code'");
      /*if($referral_point=='0')
      {
        $this->db->query("UPDATE tbl_genrate_referral_code_for_customer SET referral_point='100',referral_flag_status='1' where customer_referral_code='$referal_code' and customer_point='0'");
      }
      else
      {
        $query="insert into tbl_genrate_referral_code_for_customer(mobile_no,customer_referral_code,customer_point,referral_point,date_time,date,status,referral_flag_status) values('$mobile_no','$referal_code','0','100','$now','$nows','1','1')";
 ...
 ////////IMAGE URL fixer//////////
  ...
  function check_data_by_registerseller1_temp_moving($array,$city)
    ...
    {  
     ...
     {  
           $q=$this->db->query("SELECT DISTINCT cat_name,cat_icon_image,master_cat_id,check_level_customer from fixer_category where status='1' and type!='Tempory' and type!='Festive'  ORDER BY priority='Top1' DESC ,priority='Top2' DESC, priority='Top3' DESC, priority='Top4' DESC,priority='Top' DESC,create_date DESC");
          return($q->result_array());
     }
